...
public class UsuariosFrame extends JFrame {
	...
	public UsuariosFrame() {
		...
		btnEditar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(!table.getModel().getValueAt(sel,0).equals(""))new ModificarUsuario((String) table.getModel().getValueAt(sel,0)).setVisible(true);
			}
		});
		...
		btnEliminar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(!table.getModel().getValueAt(sel,0).equals("")) {
					int conf = JOptionPane.showConfirmDialog(null, "Realmente desea eliminar el usuario?", "Confirmar", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
					if(conf==0) {
						if(new DaoUsuario().deleteRow((String)table.getModel().getValueAt(sel,0))) {
	...
	private void cargarTabla() {
		setModel();
		List<String[]>usuarios=new DaoUsuario().getAllToList();
		for(int i=0; i<usuarios.size();i++) {
			Persona persona=(Persona) new DaoPersona().getObject(usuarios.get(i)[0]);
			Object []obj=new Object[6];
			obj[0]=usuarios.get(i)[0];
			obj[1]=persona.getNombre();
			obj[2]=persona.getApellidoPat();
			obj[3]=persona.getEmail();
			obj[4]=usuarios.get(i)[1];
			obj[5]=usuarios.get(i)[3].equals("1")?"administrador":"estandar";
			model.addRow(obj);
		}
	}
	private void buscar() {
		setModel();
		List<String[]>usuarios=new DaoUsuario().getSearchToList("usr", tfBuscar.getText());
		for(int i=0; i<usuarios.size();i++) {
			Persona persona=(Persona) new DaoPersona().getObject(usuarios.get(i)[0]);
			Object []obj=new Object[6];
			obj[0]=usuarios.get(i)[0];
		...
		for(int i=0; i<personas.size();i++) {
			Usuario aux=(Usuario) new DaoUsuario().getObject(personas.get(i)[0]);
			if(aux.getUsr()!=null) {
				Object []obj=new Object[6];
				...
				obj[4]=aux.getUsr();
				obj[5]=aux.getTipoUsuario()==1?"administrador":"estandar";
				model.addRow(obj);
			}
		}
